...
public:
    bool& visited(const int& cur) {
        return visit[cur];//compile error!!!
    }
};
```
...
public:
    ...
    void init(void) {
        ...
        mfp = &func; /*@4 참고1 사이트의 답변에는 안된다고 나와있는데
                     C++17에선 잘 되는 것 같습니다?..*/
        mfp = outFunc; // 오류!!!->mfp는 Foo클래스의 멤버함수를 가리킬 수 있는 포인터야!!!
...
private:
    //클래스 내부에서 사용시
    function<void(Foo&)> f1 = func; //&func, Foo::func, &Foo::func 전부가능
    fnnction<void(void)> f2 = outFunc;
public:
    void func(void) {
...
밖에서는 그저 함수호출에 필요한 객체를 function에 넘겨주기만하면 호출해줍니다.